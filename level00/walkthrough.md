...
Увидев в репозитории исполняемый файл, первое, что хочется сделать - посмотреть его ассемблерный код.  
Сделать это можно, например, при помощи дебаггера:  `gdb ./level00`  
Получаем вывод:  
```
(gdb) disass main
...
```
Из вывода видно, что вызывается функция `scanf` и далее полученное значение сравнивается с `0x149c (5276)`.  
```
level00@OverRide:~$ ./level00
***********************************
*            -Level00 -           *
