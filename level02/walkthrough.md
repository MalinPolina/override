...
Сделать это можно, например, при помощи дебаггера:  `gdb ./level02`  
Получаем вывод:
```
(gdb) disass main
...
(gdb)
```
Увидев в стэке вызов функции printf, сразу приходит в голову уязвимости этой функции.  
Если выводить с помощью printf лишнее количество спецификаторов (например, printf("%x %x",a)),   
то можно вторгнуться в память программы и постараться достать оттуда необходимые данные.  
Попробуем вывести первые 30 элементов:
```
level02@OverRide:~$ ./level02
===== [ Secure Access System v1.0 ] =====
/***************************************\
...
```
Так как наш пароль состоит из 40 символов, то стоит рассматривать 5 подряд идущих указателей. (40 = 5 * 8)  
Итак, в выводе нас интересуют часть:
```
0x756e505234376848
0x45414a3561733951
0x377a7143574e6758
...
0x48336750664b394d
```
Теперь попробуем перевести каждое число в ASCII и перевернуть:
```
...
```
Соединим и получим предпологаемый пароль:  
```
